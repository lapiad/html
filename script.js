...
function openLocker(lockerId) {
  ...
  if (lockers[lockerId]) {
    ...
    document.getElementById(
      "lockerOwner"
    ).innerText = `Assigned to: ${lockers[lockerId].name}`;
    document.getElementById(
      "lockerTime"
    ).innerText = `Time In: ${lockers[lockerId].timeIn}`;
  } else {
...
function lockUnlockLocker() {
  ...
  let lockerElement = document.querySelector(
    `.locker:nth-child(${selectedLocker})`
  );
  ...
  if (lockers[selectedLocker]) {
    ...
    if (
      lockers[selectedLocker].pin === pin &&
      lockers[selectedLocker].studentId === studentId
    ) {
